    ...
    <div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-1 small">
                ...
                <li class="breadcrumb-item"><a href="<?= base_url('equities') ?>" class="text-decoration-none">Equities</a></li>
                <li class="breadcrumb-item active" aria-current="page">New Stock Entry</li>
            </ol>
        </nav>
        <h3 class="fw-bold text-dark mb-0">Add New Stock Purchase</h3>
        <p class="text-secondary small mb-0">Use this form <strong>only once</strong> when adding a brand new stock to your portfolio for the first time.</p>
    </div>
    <a href="<?= base_url('equities') ?>" class="btn btn-outline-secondary btn-sm rounded-3">
        ...
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-header bg-white border-bottom py-3 px-4 rounded-top-4">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <h5 class="fw-bold text-dark mb-0">Stock & Initial Purchase Information</h5>
                        <small class="text-muted">Subsequent buys, sells, and dividends will use the quick popup form</small>
                    </div>
                </div>
            </div>

            <div class="card-body p-4 p-md-5">
                <form action="<?= base_url('equities/new') ?>" method="POST" id="newStockForm">
                    <?= csrf_field() ?>

                    <!-- Section 1: Stock Master Details -->
                    <h6 class="fw-bold text-dark text-uppercase small tracking-wide mb-3 pb-2 border-bottom">
                        <i class="bi bi-building me-1 text-primary"></i> 1. Stock Identity
                    </h6>

                    <div class="row g-3 mb-4">
                        <div class="col-md-4">
                            <label for="symbol" class="form-label small fw-semibold text-secondary">Ticker / Symbol <span class="text-danger">*</span></label>
                            <input type="text" 
                                   ...
                                   required 
                                   autofocus>
                            <div class="form-text small">NSE/BSE Trading Symbol</div>
                        </div>

                        <div class="col-md-5">
                            <label for="company_name" class="form-label small fw-semibold text-secondary">Company Name <span class="text-danger">*</span></label>
                            <input type="text" 
                                   class="form-control" 
                                   id="company_name" 
                                   name="company_name" 
                        ...
                        <div class="col-md-6">
                            <label for="sector" class="form-label small fw-semibold text-secondary">Sector / Industry</label>
                            <select class="form-select" id="sector" name="sector">
                                <option value="">-- Select Sector / Industry --</option>
                                <?php if (!empty($sectors)): ?>
                                    <?php foreach ($sectors as $sec): ?>
                                        <option value="<?= esc($sec['name']) ?>" <?= old('sector') === $sec['name'] ? 'selected' : '' ?>>
                                            <?= esc($sec['name']) ?>
                                        </option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-4">
                            <label for="quantity" class="form-label small fw-semibold text-secondary">Quantity (Shares) <span class="text-danger">*</span></label>
                            <input type="number" 
                                   class="form-control" 
                                   id="quantity" 
                                   name="quantity" 
                        ...
                        <div class="col-md-4">
                            <label for="price" class="form-label small fw-semibold text-secondary">Buy Price per Share (₹) <span class="text-danger">*</span></label>
                        ...
                        <div class="col-12">
                            <label for="notes" class="form-label small fw-semibold text-secondary">Notes / Strategy (Optional)</label>
                            <input type="text" 
                                   class="form-control" 
                                   id="notes" 
                                   name="notes" 
                                   placeholder="e.g. Long term accumulation / quarterly result dip" 
                                   value="<?= old('notes') ?>">
                        </div>
                    </div>
                    ...
                    <div class="card bg-light border-0 rounded-3 p-3 mb-4">
                        <div class="d-flex justify-content-between align-items-center mb-1">
                            ...
                            <span class="fs-5 fw-bold text-primary" id="previewTotal">₹ 0.00</span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center">
                            <span class="small text-muted">Effective Cost Per Share:</span>
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary px-4 py-2 rounded-3 fw-semibold">
                            <i class="bi bi-check2-circle me-1"></i>Save Stock & Add Initial Lot
                        </button>
                        <a href="<?= base_url('equities') ?>" class="btn btn-outline-secondary px-4 py-2 rounded-3">
                            Cancel
                        </a>
                    </div>
                </form>
